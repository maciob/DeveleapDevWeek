...
def list():
    ...
    os.system(f'echo "git checkout to dir git"')
    os.system(f"git checkout {after} git")
    os.system(f'echo "docker rm"')
    os.system("docker rm -f mysql-flask-app-container python-flask-app-container")
    os.system(f'echo "docker build db"')
    os.system("docker build . -t mysql_db:1.0  -f git/Billing/db/Dockerfile")
    os.system(f'echo "docker build billing"')
    os.system("docker build . -t billing_server:1.0 -f git/Billing/app/Dockerfile")
    os.system(f'echo "docker compose up"')
    os.system("command docker-compose --env-file ./git/Billing/config/.env.dev up --detach")
    os.system(f'echo "run the tests"')
