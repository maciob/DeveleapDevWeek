...
pushd /app/git/Billing/tests > /dev/null
    ...
    command ./get_truck_id.sh >> output.txt
    passed_count=$(grep -c "Passed" output.txt)
    if [ $passed_count -eq 0 ]; then
        echo -e "\n${bold}${red}Oh...$passed_count/10 tests passed.${white}${normal}\n"
    
    elif [ $passed_count -eq 10 ]; then
        echo -e "\n${bold}${green}$passed_count/10 tests passed! Great job!${white}${normal}\n"
